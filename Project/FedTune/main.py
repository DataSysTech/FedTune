...
if __name__ == '__main__':
    ...
    while n_cur_consecutive_better < n_consecutive_better:
        ...
        # if aggregator_name.strip().lower() == 'fedavg':
        #     # standard training
        #     fl_server.train_one_round(training_pass=E, target_client_ids=selected_client_ids)
        # elif aggregator_name.strip().lower() == 'fednova':
        #     # change client's training pass
        #     client_config = dict()
        ...
        # else:
        #     sys.exit(f'{__file__} unknown aggregator_name: {aggregator_name}')
        fl_server.train_one_round(training_pass=E, target_client_ids=selected_client_ids)
        ...
        # FedTune decisions
        if enable_FedTune:
            M, E = fedTuner.update(model_accuracy=accuracy,
                                   compT=round_compT,
                                   transT=round_transT,
                                   compL=round_compL,
                                   transL=round_transL)
